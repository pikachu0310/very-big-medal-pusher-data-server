...
function HomePage() {
  ...
  const [chaosLocked, setChaosLocked] = useState(false);
  ...
  const [jackpotFeverText, setJackpotFeverText] = useState('JACKPOT FEVER');
  const [achievementCelebrationVisible, setAchievementCelebrationVisible] = useState(false);
  const [achievementCelebrationDone, setAchievementCelebrationDone] = useState(false);
  ...
  const spawnAdPopup = useCallback((text?: string) => {
    const popup: AdPopup = {
      id: ++adIdRef.current,
      text: text ?? randomFrom(fakeAdMessages),
      x: 6 + Math.random() * 82,
      y: 8 + Math.random() * 78
    };
  ...
  useEffect(() => {
    ...
    setAchievementCelebrationDone(true);
    setAchievementCelebrationVisible(true);
    triggerCoinRain(260, 0, 5200);
    pushFakeAlert();
    spawnAdPopup('実績コンプリートおめでとう！！！ ここまで来ると、もう運営側です。');

    const timerId = window.setTimeout(() => {
      setAchievementCelebrationVisible(false);
    }, 5200);
    burstTimersRef.current.push(timerId);
  }, [achievementCelebrationDone, completedAchievements, pushFakeAlert, spawnAdPopup, triggerCoinRain]);
  ...
  useEffect(() => {
    const timerId = window.setInterval(() => {
      setJackpotCount((current) => {
        const growth = 1.01 + (chaosLevel / 1000) + (chaosOverdrive ? 0.01 : 0);
        const bonus = 1200 + Math.floor(Math.random() * 3200);
        return Math.floor(current * growth + bonus);
      });
    }, 1700);
  ...
  const rollFortune = () => {
    const result = randomFrom(fortunePool);
    setTodayFortune(result);
    registerChaosAction(4);

    if (result.includes('Heaven Jackpot')) {
      updateAchievement('heavenJackpot');
      setJackpotFeverText('Heaven Jackpot！！！');
      setJackpotFever(true);
      triggerCoinRain(220, 0, 4500);
      pushFakeAlert();
      spawnAdPopup('Heaven Jackpot！！！ おめでとう！！！');
      const feverTimer = window.setTimeout(() => setJackpotFever(false), 4500);
    ...
    >
      ...
      <section className="april-gimmick-grid" aria-label="イベントギミック">
        ...
        <article className="april-card">
          ...
          <div className="april-boss-hp" aria-hidden="true">
            <span style={{ width: `${(bossHp / bossMaxHp) * 100}%` }} />
          </div>
          <p className="april-boss-percent">ボスHP: {bossHp.toLocaleString()} / {bossMaxHp.toLocaleString()}</p>
          {bossHp > 0 ? (
      ...
      {maintenanceVisible && (
        <div className="april-maintenance-overlay" role="alertdialog" aria-modal="true" aria-live="assertive">
          <div className="april-maintenance-card">
            ...
            <span>※大体「あ、やべ！」みたいなミスで発生する</span>
          </div>
        </div>
      )}

      {adPopups.length > 0 && (
        <div className="april-ad-cloud" aria-live="polite">
          {adPopups.map((popup) => (
            <div
              key={popup.id}
              className="april-ad-popup"
            ...
            >
              <div className="april-ad-card">
                ...
                <button type="button" onClick={() => removeAdPopup(popup.id)}>
                  閉じる
                </button>
              </div>
            </div>
      ...
      {jackpotFever && (
        <div className="april-jackpot-fever" aria-hidden="true">
          <p>{jackpotFeverText}</p>
        </div>
      )}

      {achievementCelebrationVisible && (
        <div className="april-achievement-complete" aria-hidden="true">
          <p>実績コンプリートおめでとう！！！</p>
          <span>ここまで来たあなたは、もう半分運営スタッフです。</span>
        </div>
      )}
    </div>
  );
